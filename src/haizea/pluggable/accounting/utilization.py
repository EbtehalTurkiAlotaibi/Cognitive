...
from haizea.pluggable.accounting.models import Experiment, CPUPnode, DiskPnode,\
    NetInPnode, NetOutPnode, MemoryPnode, CPU
...
class CPUUtilizationProbe(AccountingProbe):
    ...
    * Counters
    ...
    """    
    COUNTER_UTILIZATION = "AVG CPU utilization"        
    COUNTER_INTERVAL = "INTERVAL COUNTER"
    ...
    def finalize_accounting(self, db):
        ...
        experiment.total_cpu_util = self.total_utilization
        experiment.cpu_util_interval = self.count

        db.execute(CPU.__table__.insert(), cpu_utils)
        db.commit()
