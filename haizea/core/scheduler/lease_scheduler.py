...
from haizea.core.leases import Lease, Timestamp
...
#############################################################################################################################
#############################################################################################################################
#			Second change (2): process cognitive leases directly 						    #
#					Second Attept									    #
#############################################################################################################################
#############################################################################################################################
"""
class Cognitive(threading.Thread):
    def __init__(self,lease, nexttime,vm_scheduler, preparation_scheduler, slottable, accounting):
        threading.Thread.__init__( self, name = str(lease.id) )
	self.lease = lease
    ...
    def run(self):
	self.lease.print_contents()
	try:
		self.leasescheduler.__schedule_lease(self.lease, nexttime=self.nexttime)
                self.leasescheduler.logger.info("Cognitive lease lease #%i has been scheduled." % self.lease.id)
                self.lease.print_contents()
	except NotSchedulableException, exc:
		# Exception parameter set to save the node we tried to 
		self.leasescheduler.logger.info("Cognitive lease lease request #%i cannot be scheduled now a time, it will be queued on node" % (self.lease.id))
		self.lease.start = Timestamp(Timestamp.UNSPECIFIED)
		self.lease.preemptible = True
		self.lease.cognitive = False                
		self.leasescheduler.__enqueue(self.lease)
		self.lease.set_state(Lease.STATE_QUEUED)
		self.leasescheduler.logger.info("Queued Cognitive lease request #%i, %i nodes for %s." % (lease.id, lease.numnodes, lease.duration.requested))
...
#						Helper function							   #
#******************************************************************************************************************#
    def add_overhead (self,nexttime,msec):
	time = datetime.strptime(str(nexttime),"%Y-%m-%d %H:%M:%S.%f")
	year = time.year
	month = time.month
	day = time.day
	H = time.hour
	...
	if (month >=13 ):
		...
		month = month%12
	if (year > 2020 or year < 2014 ):
		year = 2014
	nexttime = DateTime.Timestamp(year,month,day,H, M,S)
	return nexttime 
#******************************************************************************************************************#
...
#############################################################################################################################
		...
		lease.queuingtime = lease.queuingtime + (lease.queue_endtime - lease.queue_starttime)
